...
const VoteForMissionStart: React.FC<VoteForMissionStartProps> = ({ sessionId, missionId, missionParticipants }) => {
  ...
  const handleVote = async (vote: 'succeed' | 'fail') => {
    ...
    try {
      const response = await fetch(`/api/session/${sessionId}/mission/${missionId}/startMissionVotes`, {
        method: 'POST',
        headers: {
  ...
  return (
    <div className="flex flex-col items-center p-4">
      <h2 className="text-lg font-semibold mb-2">Vote For Mission Start</h2>
      <h2 className="text-base font-semibold text-gray-500">{missionParticipants.join(', ')} will go on a mission</h2>
...
export default VoteForMissionStart;
